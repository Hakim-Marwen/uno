...
function getWsUrl() {
  ...
  // Connexion WS sur le même domaine/port que le serveur Express
  return `${proto}//${host}`;
}
...
function connect() {
  ...
  state.ws.onopen = () => {
    console.log('[WS] Connecté à', getWsUrl());
    state.connected = true;
    state.reconnectAttempts = 0;
    hideOverlay('disconnect');
    clearInterval(state.pingInterval);
    state.pingInterval = setInterval(sendPing, 30000);
    // Le serveur enverra CONNECTED pour confirmer, puis on passe au lobby
    // Mais on affiche le lobby directement si pas de salle en cours
    if (!state.roomCode) {
      showScreen('lobby');
    }
  };
...
function handleServerMessage(msg) {
  switch (msg.type) {
    case 'CONNECTED':
      // Serveur confirmé, aller au lobby
      console.log('[WS] Serveur connecté, ID=', msg.id);
      break;
    ...
    case 'ROOM_CREATED':
      ...
      state.isHost = true;
      console.log('[CLIENT] Salle créée, myId=', state.myId, 'hostId=', msg.room.hostId);
    ...
    case 'ROOM_JOINED':
      ...
      console.log('[CLIENT] Salle rejointe, myId=', state.myId, 'hostId=', msg.room.hostId, 'isHost=', state.isHost);
      renderWaiting(msg.room);
      showScreen('waiting');
      break;
...
function handleGameState(room) {
  ...
  if (room.phase === 'waiting') {
    ...
    if (getCurrentScreen() !== 'game') showScreen('game');
    renderGame(room);
    // Eviter de re-afficher si déjà visible
...
document.getElementById('btn-create').addEventListener('click', () => {
  ...
  setTimeout(() => { document.getElementById('btn-create').disabled = false; }, 3000);
});
...
document.getElementById('btn-join').addEventListener('click', () => {
  ...
  setTimeout(() => { document.getElementById('btn-join').disabled = false; }, 3000);
});
...
function renderWaiting(room) {
  ...
  for (const p of room.players) {
    ...
    const isMe = p.id === state.myId ? ' (vous)' : '';
    html += `<div class="player-item">
      <div class="player-avatar" style="background:${p.avatarColor}25;color:${p.avatarColor}">${initials}</div>
      <span class="player-name">${escHtml(p.name)}${isMe}</span>
      ${badge}
    </div>`;
  }
  list.innerHTML = html;

  // Mettre à jour isHost depuis l'état serveur (source de vérité)
  state.isHost = (room.hostId === state.myId);

  console.log('[renderWaiting] myId=', state.myId, 'hostId=', room.hostId, 'isHost=', state.isHost, 'players=', room.players.length);
  ...
  startBtn.textContent = room.players.length >= 2
    ? `▶ Démarrer (${room.players.length} joueurs)`
    : `▶ Démarrer (${room.players.length}/2 min.)`;
